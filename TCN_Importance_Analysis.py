# #!/usr/bin/env python3
#     """
#     The main function performs training, evaluation, and visualization of a Temporal Convolutional
#     Network (TCN) model for binary classification on multiple datasets, including data preprocessing,
#     model training, evaluation metrics calculation, and visualization of results.
    
#     :param arr_ch_t: `arr_ch_t` is a numpy array representing the input data for the model. It has a
#     shape of (C, T) where C is the number of channels and T is the number of time steps
#     :type arr_ch_t: np.ndarray
#     :param label: The label is the target variable in your dataset that you want to predict. In this
#     case, it represents the class or category that each data point belongs to. In your code, the labels
#     are binary (0 or 1) where 0 represents one class (LCL) and 1 represents
#     :type label: int
#     :param window_size: The `window_size` parameter refers to the size of the window used for segmenting
#     the input data. In the context of the provided code, it represents the number of data points or time
#     steps included in each window of the input data. This parameter is crucial for creating input
#     sequences for the TCN, defaults to 240
#     :type window_size: int (optional)
#     :param overlap: The `overlap` parameter specifies the amount of overlap between consecutive windows
#     when windowizing the data. In the provided script, it is set to `0.0`, which means there is no
#     overlap between windows. If you want to introduce some overlap between windows, you can set this
#     parameter to a value
#     :type overlap: float
#     :return: The script is returning various outputs and visualizations related to the training and
#     evaluation of a Temporal Convolutional Network (TCN) model for binary classification. Here are the
#     main outputs being returned:
#     """
...
def gradcam_feature_importance(model, device, test_loader, feature_names, out_dir):
    ...
    for xb, yb in test_loader:
        ...
        loss = out.gather(1, pred_class.unsqueeze(1)).sum()
        loss.backward()

        input_grad = xb.grad.cpu().numpy()
        importance = np.mean(np.abs(input_grad), axis=(0, 2))
    ...
    feat_importance /= count

    idx = np.argsort(feat_importance)[::-1]
    top_names = [feature_names[i] for i in idx]
    top_vals = feat_importance[idx]

    plt.rcParams['font.family'] = 'Times New Roman'
    plt.rcParams['axes.unicode_minus'] = False
    plt.figure(figsize=(10, 5))
    plt.barh(top_names[:10], top_vals[:10], color='#2E86AB')
    plt.xlabel('Feature Importance (Gradient)', fontsize=12)
    plt.title('Top 10 Important Eye-Tracking Features', fontsize=13)
    ...
    plt.close()
    np.savetxt(os.path.join(out_dir, "feature_importance.txt"), feat_importance, fmt="%.6f")
...
def run_fold(model, device, train_loader, val_loader, test_loader,
    ...
    return np.array(y_true), np.array(y_pred), np.array(y_prob1), feats_arr, labels_arr, fold_feat_score
...
def main():
    ...
    for dtype in datasets_req:
        ...
        for k in range(5):
            ...
            # —— 训练（带验证/早停/最优模型保存） —— #
            y_true, y_pred, y_prob1, feats_fold, labels_fold, fold_feat_score = run_fold(
            ...
            if fold_feat_score is not None:
                fold_feature_scores.append(fold_feat_score)
        ...
        # ===================== 5折平均特征重要性总图 =====================
        if len(fold_feature_scores) > 0:
            mean_feat = np.mean(np.array(fold_feature_scores), axis=0)
            ...
            plt.barh(top_names[:10], top_vals[:10], color='#A23B72')
            plt.xlabel('Average Feature Importance (5-Fold)', fontsize=12)
            plt.title(f'5-Fold Average Top 10 Features ({dtype})', fontsize=13)
            plt.xticks(fontsize=11)
            plt.yticks(fontsize=11)
            plt.gca().invert_yaxis()
            plt.tight_layout()

            plt.savefig(os.path.join(res_dir, "5fold_average_feature.png"), dpi=300, bbox_inches='tight')
            plt.close()
            np.savetxt(os.path.join(res_dir, "5fold_average_feature.txt"), mean_feat, fmt="%.6f")
