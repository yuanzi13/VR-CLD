...
def plot_binary_roc_compare(entries, save_path, title="LCL vs HCL ROC Comparison"):
    ...
    for i, (name, y_true_all, y_prob_all) in enumerate(entries):
        ...
        try:
            ...
            print(f"  ⚠️ {name} ROC 失败: {e}")
    if plotted == 0:
        print("  ⚠️ 无满足条件的组，跳过比较 ROC")
        return
    plt.plot([0,1],[0,1],'k--', lw=1)
    plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate')
    plt.title(title)
...
def main():
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        f.write("本次运行关键设置：\n")
        f.write(f"- LOSO 评估方案\n")
        f.write(f"- 窗口化(window={args.window_size}, overlap={args.overlap})\n")
    ...
    for dtype in datasets_req:
        ...
        for key in valid_keys:
            ...
            # ===================== 接收 6 个返回值 =====================
            y_true, y_pred, y_prob, feats_fold, labels_fold, fold_feat_score = run_fold(
            ...
            plot_multiclass_roc(y_true, y_prob, n_classes=num_classes,
                title=f"{dtype} TCN LOSO {pop}-{sid:02d} ROC (OVR)",
                save_path=os.path.join(fold_dir, "roc.png"), include_micro=False)
        ...
        if len(fold_feature_scores) > 0:
            ...
            plt.xlabel('Average Feature Importance (LOSO All Subjects)', fontsize=12)
            plt.title(f'LOSO Average Top 10 Features ({dtype})', fontsize=13)
            plt.gca().invert_yaxis()
            plt.tight_layout()
            plt.savefig(os.path.join(res_dir, "LOSO_average_feature.png"), dpi=300, bbox_inches='tight')
            plt.close()
            np.savetxt(os.path.join(res_dir, "LOSO_average_feature.txt"), mean_feat, fmt="%.6f")
        ...
        if y_t_all:
            ...
            plot_multiclass_roc(np.array(y_t_all), np.array(y_pr_all), n_classes=num_classes,
                title=f"{dtype} TCN LOSO Overall ROC (OVR)",
                save_path=os.path.join(res_dir, "roc_overall.png"), include_micro=False)
